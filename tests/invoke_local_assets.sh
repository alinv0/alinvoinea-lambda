...
cargo lambda invoke --data-ascii '{
  "httpMethod": "POST",
  "body": "{\"action\":\"Query\",\"query\":\"query GetSimilarPosts($categories: [String!], $slug: String!) { posts( orderBy: createdAt_DESC first: 5 where: { categories_some: { name_in: $categories }, slug_not: $slug } ) { title featuredImage { url } createdAt slug } }\",\"variables\":{\"categories\":[\"Web Development\"],\"slug\":\"genai-evolving\"}}",
  "requestContext": {
